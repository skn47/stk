...
fn no_child_process_is_spawned_for_an_unsupported_command() {
    ...
    cli::run(
        &["frobnicate".to_string(), "check".to_string()],
        std::io::empty(),
        &mut stdout,
        &mut stderr,
        &executor,
    ...
    assert_eq!(
        ...
        0,
        "no specialist is registered for 'frobnicate', so it must not reach the executor"
    );
}

#[test]
fn a_registered_specialist_command_does_reach_the_executor() {
    let executor = FakeExecutor::new();
    let history = FakeHistoryStore::new();
    let mut stdout = Vec::new();
    let mut stderr = Vec::new();
    ...
    assert_eq!(
        ...
        "cargo is a registered Specialist, so stk cargo check must dispatch to it"
    );
}
